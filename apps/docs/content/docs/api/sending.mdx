...
result = wm.emails.send(
    from_address="notifications@wistfare.com",
    to=["user@example.com"],
    subject="Your order has shipped",
    html="""
    <h1>Order Shipped!</h1>
...
<Tab value="cURL">
```bash
curl -X POST https://mail.wistfare.com/api/v1/emails/batch \
  -H "X-API-Key: wm_your_api_key" \
  -H "Content-Type: application/json" \
  -d '{
    "emails": [
...
<Tab value="cURL">
```bash
curl -X PATCH https://mail.wistfare.com/api/v1/emails/eml_abc123/cancel \
  -H "X-API-Key: wm_your_api_key"
```
</Tab>
</Tabs>
