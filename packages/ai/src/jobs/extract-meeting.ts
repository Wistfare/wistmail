...
/**
 * Pull a meeting (one — not many) out of an inbound email when the
 * sender clearly proposes or confirms one. Caller decides what to do
 * with the result by confidence band:
 *
 *   ≥ 0.85  auto-create a calendar_events row, link back to the email
 *   0.60–0.85  surface "Add to calendar?" chip in the UI
 *   < 0.60  store the extraction silently (analytics) — no UI prompt
 *
 * The model is asked to resolve relative dates ("tomorrow", "this
 * Friday") against the email's `sentAtIso`, so the caller doesn't
 * need its own date parser. ISO 8601 with offset is required for
 * auto-create paths — the worker validates parseability before
...
export async function extractMeeting(
  ...
  const json = result.json as Partial<ExtractMeetingOutput> | undefined
  ...
  return {
    ...
    location: typeof json.location === 'string' ? json.location.slice(0, 500) : null,
